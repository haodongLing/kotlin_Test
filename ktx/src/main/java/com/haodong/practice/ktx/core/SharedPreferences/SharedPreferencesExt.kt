package com.haodong.practice.ktx.core.SharedPreferences
 ...
 * @param name Desired preferences file. Default value is the packageName
 */
fun <T> Context.putSpValue(key: String, value: T,commit: Boolean=false, name: String = packageName) =sp(name).edit(commit){
    when (value) {
...
fun <T> Activity.putSpValue(key: String, value: T,commit: Boolean=false, name: String = packageName) = sp(name).edit (commit){
    when (value) {
        is Long -> putLong(key, value)
        is String -> putString(key, value)
        is Int -> putInt(key, value)
