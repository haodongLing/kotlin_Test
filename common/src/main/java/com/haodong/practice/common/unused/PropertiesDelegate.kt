package com.haodong.practice.common.unused
...
class PropertiesDelegate(private val path: String) {
    ...
    private val properties: Properties by lazy {
        val prop = Properties()
        url = try {
            javaClass.getResourceAsStream(path).use { prop.load(it) }
            javaClass.getResource(path)
        } catch (e: Exception) {
            try {
                ClassLoader.getSystemClassLoader().getResourceAsStream(path).use { prop.load(it) }
    ...
    operator fun <T> getValue(thisRef: Any, property: KProperty<*>): T {
        ...
        val classOfT = property.returnType.classifier as KClass<*> // field 类型
        return when {
            ...
            else -> throw IllegalArgumentException("unSupposed type .")
        } as T
    }
